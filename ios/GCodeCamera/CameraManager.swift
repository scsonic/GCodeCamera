    ...
    */
    ...
    private func _addVideoInput()
    {
            ...
            if (self.frontCamera == nil) || (self.rearCamera == nil) {
                ...
                if (self.frontCamera == nil) {
                    if let validVideoFrontDevice = videoFrontDevice {
                        ...
                        self.frontCamera = try AVCaptureDeviceInput(device: validVideoFrontDevice )
                    }
                }
                if (self.rearCamera == nil) {
                    if let validVideoBackDevice = videoBackDevice {
                        self.rearCamera = try AVCaptureDeviceInput(device: validVideoBackDevice)
                    }
                }
            }
        ...
        self.cameraDevice = _cameraDevice
            ...
            self._show(NSLocalizedString("Device setup error occured", comment:""), message: "error")
    ...
    private func _setupMic()
    {
        do {
            if (self.mic == nil) {
                ...
                self.mic = try AVCaptureDeviceInput(device: micDevice)
            }
        }
        catch _ {
            self.mic = nil
            self._show(NSLocalizedString("Mic error", comment:""), message: "setup mic error")
        }
    }
